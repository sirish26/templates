...
export default function Blog() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 sm:px-6 sm:py-24">
      <h1 className="text-5xl sm:text-7xl font-black uppercase tracking-tighter mb-10 sm:mb-16">Writing</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        {posts.map(post => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
            <div className="aspect-[4/3] bg-white border-4 border-black mb-6 group-hover:bg-black transition-colors flex items-center justify-center">
            ...
            <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-tight group-hover:underline">{post.title}</h2>
          </Link>
        ))}
      </div>
    </div>
